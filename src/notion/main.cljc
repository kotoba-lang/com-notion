(ns notion.main
  "Kotodama WASM entrypoint for the Notion clean-room actor (L4) — Clojure port.
  ...
  facts (`notion.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "notion")
(def tier "L4")
...
(def entity-specs
  [{:entity "Page"     :plural "pages"     :id-prefix "notion_pag"
    :fields [:object :createdTime :lastEditedTime :inTrash :url] :required [:object :createdTime]
    :coerce {}                       :refs {}}
   {:entity "Database" :plural "databases" :id-prefix "notion_dat"
    :fields [:object :title :description :isInline :url]           :required [:object :title]
    :coerce {:isInline :bool}        :refs {}}
   {:entity "Block"    :plural "blocks"    :id-prefix "notion_blo"
    :fields [:object :type :hasChildren]                           :required [:object :type]
    :coerce {:hasChildren :bool}     :refs {}}
   {:entity "User"     :plural "users"     :id-prefix "notion_use"
    :fields [:object :name :type :avatarUrl]                       :required [:object :name]
    :coerce {}                       :refs {}}
   {:entity "Comment"  :plural "comments"  :id-prefix "notion_com"
    :fields [:discussionId :createdTime]                           :required [:createdTime]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"notion.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "notion-compat" :tier tier :entities entities} 200])
...
(defn start! [] :notion-compat/ready)
